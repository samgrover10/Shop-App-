...
class _AuthCardState extends State<AuthCard>
  ...
  void initState() {
    ...
    _slideAnimation = Tween(begin: Offset(0.0, -1.25), end: Offset(0.0, 0.0))
        .animate(CurvedAnimation(parent: _controller, curve: Curves.easeIn));
  }
  ...
  void _submit() async {
    ...
    try {
      ...
      var errMessage = 'Authentication error';
      if (httperror.toString().contains('EMAIL_NOT_FOUND')) {
        errMessage = 'User account does not exist';
      } else if (httperror.toString().contains('INVALID_PASSWORD')) {
        errMessage = 'Invalid password!';
      } else if (httperror.toString().contains('EMAIL_EXISTS')) {
        errMessage = 'User account already exixts!';
      } else if (httperror.toString().contains('INVALID_EMAIL')) {
        errMessage = 'Invalid email';
      }
      _showErrorDialog(errMessage);
    } catch (e) {
      _showErrorDialog('Authentication or Network error ,Try again later.');
    }
    setState(() {
      _isLoading = false;
    });
